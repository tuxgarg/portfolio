<script lang="ts">
  ...
  import urhome from '$lib/assets/icons/urhome.png';
  ...
  function handleDockClick(appType: string) {
    if (!isLargeScreen && appType !== 'github' && appType !== 'urhome') {
      showPopup = true;
    } else {
      if (appType === 'launchpad') {
        dispatch('openLaunchpad');
      } else if (appType === 'github') {
        window.open('https://github.com/tuxgarg', '_blank');
      } else if(appType === 'urhome'){
        window.open('https://chromewebstore.google.com/detail/urhome-productive-workspa/nlplbpekjjbgdggafhnodmpfhfhdflij', '_blank');
      } else {
