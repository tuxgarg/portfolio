<script lang="ts">
  ...
  function processCommand(cmd: string) {
    addLine(
      `<span class="text-green-400">tushar@macbook</span><span class="text-white">:</span><span class="text-blue-400">~/${currentDirectory.join("/")}</span><span class="text-white">$</span> ${cmd}`
    );
  ...
  onMount(() => {
    ...
    showPrompt = true;
    addLine("Welcome to tushar's Terminal");
...
<div class="bg-gray-900/40 backdrop-blur-md h-full border border-white/20">
  ...
  >
    ...
    <div class="flex-grow text-center text-sm text-gray-400">
      tushar@macbook: ~/{currentDirectory.join("/")}
    </div>
  </div>
  ...
  <div
    ...
    class="bg-black/30 p-4 overflow-y-auto text-wrap scrollbar-hidden"
    style="height: calc(100% - 80px);"
  >
    {#each terminalLines as line}
      <br />
      <span
        class="text-gray-200 whitespace-pre text-wrap"
  ...
  {#if showPrompt}
    <div class="bg-black/30 px-4 py-2 flex items-center">
      <span class="text-green-400 mr-1">tushar@macbook</span>
